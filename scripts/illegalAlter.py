...
def isMySQL8():
    database = script_helper.get_database()
...
#
logger = script_helper.get_logger()
...
script_name = script_helper.get_script_path()
...
#
snapshot = script_helper.get_snapshot()
...
for change in changes:
    sql = script_helper.generate_sql(change)
    sql = script_helper.strip_comments(sql)
    logger.info("Processing SQL " + sql)
    statements = script_helper.split_statements(sql)
    for statement in statements:
        #
        # Break into tokens
        #
        tokens = script_helper.tokenize(statement)
        ...
        if col_size < 256 and i_mod_size >= 256:
            status = script_helper.get_status()
            status.fired = True
            status.message = script_helper.get_script_message()
